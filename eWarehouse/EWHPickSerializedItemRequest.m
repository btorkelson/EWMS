...
-(void)pickItem:(EWHShipmentDetail *)shipmentDetail warehouse:(NSInteger)warehouseId location:(EWHLocation *)location locationName:(NSString *)locationName serialNumbers:(NSString *)serialNumbers user:(EWHUser *)user
{
    ...
    NSString *postData = [NSString stringWithFormat:@"{\"item\":{\"WarehouseId\":%d,\"ShipmentId\":%d,\"LocationName\":\"%@\",\"LocationId\":%d,\"PartNumber\":\"%@\",\"Quantity\":\"1\",\"Number\":\"%@\",\"IsSerialized\":true},{\"userId\":\"%d\"}}", warehouseId, shipmentDetail.ShipmentId, locationName, location.Id, shipmentDetail.PartNumber, serialNumbers, user.UserId];

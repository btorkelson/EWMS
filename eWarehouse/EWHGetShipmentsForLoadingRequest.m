//
//  EWHGetShipmentsForLoadingRequest.m
//  eWarehouse
//
...
@implementation EWHGetShipmentsForLoadingRequest

-(void)getShipmentsForLoadingRequest:(NSInteger)warehouseId withAuthHash:(NSString *)authHash
{
    __weak EWHRequest *sender = self;
    NSString *url = [NSString stringWithFormat:@"%@%@", super.baseURL, @"/GetShipmentsForLoading"];
    ...
    [request setCompletionBlock:^{
        ...
        if(code == 200){
            ...
            for (NSDictionary* element in [dictionary objectForKey:@"GetShipmentsForLoadingResult"]) {
                EWHShipment* shipment = [[EWHShipment alloc] initWithDictionary:element];
                [shipments addObject:shipment];
            }            
            if(self.caller && self.callback){
                if([self.caller respondsToSelector:self.callback]){
                    [self.caller performSelector:self.callback withObject:shipments];
                }
            }
        }

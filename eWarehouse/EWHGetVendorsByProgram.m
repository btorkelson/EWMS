...
@implementation EWHGetVendorsByProgram
...
//    __weak EWHRequest *sender = self;
    NSString *url = [NSString stringWithFormat:@"%@%@", super.baseURL, @"/GetProgramVendors"];
...
//    NSError *e = nil;
        ...
        NSMutableArray* vendors = [[NSMutableArray alloc] init];
            for (NSDictionary* element in [dictionary objectForKey:@"GetProgramVendorsListResult"]) {
                EWHVendor* vendor = [[EWHVendor alloc] initWithDictionary:element];
                [vendors addObject:vendor];
            }
        if(self.caller && self.callback){
            if([self.caller respondsToSelector:self.callback]){
                [self.caller performSelector:self.callback withObject:vendors];
            }
        }

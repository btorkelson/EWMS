//
//  EWHShipmentDetailsController.m
//  eWarehouse
//
...
@implementation EWHShipmentDetailsController
{
...
-(void) loadShipmentDetails
{
    ...
    if(user != nil){
        EWHGetShipmentDetailsForPickingRequest *request = [[EWHGetShipmentDetailsForPickingRequest alloc] initWithCallbacks:self callback:@selector(getShipmentDetailsCallback:) errorCallback:@selector(errorCallback:) accessDeniedCallback:@selector(accessDeniedCallback)];
        [request getShipmentDetailsForPickingRequest:shipment.ShipmentId withAuthHash:user.AuthHash];
    }
}

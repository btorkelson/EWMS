//
//  EWHGetShipmentDetailsforPickingRequestNEW.m
//  eWarehouse
//
...
@implementation EWHGetShipmentDetailsforPickingRequestNEW
...
-(void)getShipmentDetailsForPickingRequestNEW:(NSInteger)shipmentId location:(NSString *)location withAuthHash:(NSString *)authHash
{
    ...
    NSString *url = [NSString stringWithFormat:@"%@%@", super.baseURL, @"/GetShipmentDetailsForPickingByLocation"];
    request = [ASIHTTPRequest requestWithURL:[NSURL URLWithString:url]];
    NSString *postData = [NSString stringWithFormat:@"{\"shipmentId\":\"%d\",\"location\":\"%@\"}", shipmentId, location];
    EWHLog(@"GetLocationsForPicking: %@", postData);
    ...
    [request setCompletionBlock:^{
        ...
        if(code == 200){
            ...
            for (NSDictionary* element in [dictionary objectForKey:@"GetShipmentDetailsForPickingByLocationResult"]) {

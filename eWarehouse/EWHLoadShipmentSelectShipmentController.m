//
//  EWHLoadShipmentSelectShipmentController.m
//  eWarehouse
//
...
@implementation EWHLoadShipmentSelectShipmentController
{
...
-(void) loadShipments
{
    ...
    if(user != nil){
        EWHGetShipmentsForLoadingRequest *request = [[EWHGetShipmentsForLoadingRequest alloc] initWithCallbacks:self callback:@selector(getShipmentListCallback:) errorCallback:@selector(errorCallback:) accessDeniedCallback:@selector(accessDeniedCallback)];
        [request getShipmentsForLoadingRequest:warehouse.Id withAuthHash:user.AuthHash];
    }
}
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    EWHShipment *shipment = [shipments objectAtIndex:indexPath.row];
    [self performSegueWithIdentifier:@"ShowShipmentDetails" sender:shipment];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([[segue identifier] isEqualToString:@"ShowShipmentDetails"]) {
        EWHLoadShipmentShipmentDetailsController *shipmentDetailsController = [segue destinationViewController];
        shipmentDetailsController.shipment = sender;
        shipmentDetailsController.warehouse = warehouse;
    }
}

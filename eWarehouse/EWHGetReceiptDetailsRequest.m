...
@implementation EWHGetReceiptDetailsRequest

-(void)getReceiptDetails:(NSInteger)receiptId withAuthHash:(NSString *)authHash
{
//    __weak EWHRequest *sender = self;
    NSString *url = [NSString stringWithFormat:@"%@%@", super.baseURL, @"/GetReceiptDetails"];
//    request = [ASIHTTPRequest requestWithURL:[NSURL URLWithString:url]];
    NSString *postData = [NSString stringWithFormat:@"{\"receiptId\":\"%d\"}", receiptId];
    ...
    //    NSError *e = nil;
            ...
            for (NSDictionary* element in [dictionary objectForKey:@"GetReceiptDetailsResult"]) {
                EWHReceiptDetail* receipt = [[EWHReceiptDetail alloc] initWithDictionary:element];
...
//        EWHLog(@"Response:%@", responseString);
//        if(code == 200){
